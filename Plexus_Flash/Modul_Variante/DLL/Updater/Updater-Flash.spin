...
Flash_HD  = $20000 '..$21000        'Kopie des Flash-Updatespeichers für Updateprozedur
...
RETURN_KEY   = 13
'--------------------------------------------------------------------------------
Filenum      = 52               'Anzahl der updatefähigen Dateien
var
...
'################### Update-Dateien ##################################################################
   ...
   File27        byte "TRIOSBEL.FLS",0          '$D8000                         'TRIOS-Grafiktreiber
   File28        byte "PLEXBEL.FLS",0           '$E0000                         'Plexus-Grafiktreiber
...
'################ BASIC-ADRESSEN ############################################################################
   File64        byte "BASIC3.BIN",0            '$200000
   File65        byte "MODE0.SYS",0             '$208000
   File66        byte "MODE1.SYS",0             '$210000
   File67        byte "MODE2.SYS",0             '$218000
   File68        byte "MODE3.SYS",0             '$220000
   File69        byte "MODE4.SYS",0             '$228000
   File70        byte "IMPORT.SYS",0            '$230000
   File71        byte "EXPORT.SYS",0            '$238000
   File72        byte "ERRORS.TXT",0            '$240000
   File73        byte "MODE5.SYS",0             '$248000
   File74        byte "SYSFONTB.FNT",0          '$2F0000
   ...
   FFILE word @File0,@File1,@File2,@File3,@File4,@File5,@File6,@File7,@File8,@File9,@File10
         ...
         word @File64,@File65,@File66,@File67,@File68,@File69,@File70,@File71,@File72,@File73,@File74
   ...
   FADR  LONG $0,$8000,$10000,$18000,$20000,$28000,$30000,$38000,$40000,$48000,$50000,$58000,$60000
         ...
         LONG $200000,$208000,$210000,$218000,$220000,$228000,$230000,$238000,$240000,$248000,$2F0000     'Basic-Adressen

   FSIZE byte 8,8,8,8,8,8,8,8,8,8
         byte 8,8,8,8,8,8,8,8,8,8
         byte 8,8,8,8,8,8,8,8,8,8
         byte 8,8,1,3,3,1,1,1,1,1                                                    'Bereiche*$1000
         byte 8,8,8,8,8,8,8,8,8,8,8                                                  'Basic
   ...
   Flash_Done     byte "Update-Process done !",0
   RDUPDATE_text  byte "READ Update-Informations...   ",0

PUB main|err,m

    'ios.upperstart                               'nur aktivieren, wenn Updater per RamUpLoad von BST zu Regnatix geladen wird weil er nicht mehr regulär funktioniert
    ios.start
  ...
  repeat  'while (stradr <> 0)
         ...
         if stradr<>0                                                                                                     'wiederhole bis verzeichnisende
            ...
            if c>-1
               ...
               if update_marker[c]>0                       'Datumsvergleich , damit man auch ältere Versionen flashen kann
                  updatefiles++
               n++
            c:=-1
         else
...
pri copy_update_flash(mode)|i,adr,f_adr
    ...
    if mode
       ...
       ios.erase_Flash_Data(Flash_Dat)
       waitcnt(clkfreq+cnt)
       repeat $1000                                      'Datenbereich schreiben
           ios.write_Flash_data(f_adr++,ios.ram_rdbyte(adr++))
       ios.mousepointer_flash(Mouse_Rom)
       messagebox(@Flash_Done,1)
       abfrage
       ios.restore_area(popupx,popupy,popupxx,popupyy,BRAM)
       ios.display2dbox(colors[winhcol],5,3,5,30,0)
       'ausstieg
    else
       printfont(@RDUPDATE_text,colors[winhcol],0,colors[act_color],3,5)
       ios.flxgetblk(adr,f_adr,$1000)
...
pri compare_string(str):d|i,c
    ...
    repeat i from 0 to Filenum                          'Dateinamen mit Updateliste vergleichen
...
pri Write_Flash_Files|i,adr,p,n,v,b,pr
    i:=0
    v:=0
    b:=100/updatefiles          'Schrittweite für Prozentanzeige
    pr:=0
    if updatefiles>0
       messagebox(string("Perform update?"),1)
       v:=abfrage
    else
    ...
    if v
       ...
       repeat i from 0 to filenum-1
              if update_marker[i]==1
                 ...
                 pr+=b
                 printdec(pr,5,33,colors[winhcol],colors[act_color])                   'Prozent-Fortschrittsanzeige
                 printfont(string("% "),colors[winhcol],0,colors[act_color],36,5)
...
PRI Display_Info
    ...
    Print_win(string("Version 1.4 - 07/2019"),9,11)
...
pri ausstieg
    ios.mousepointer_flash(hour_glass)
    if ios.ram_rdbyte(START_FLAG)==236                                           'Warm-Start-Flag
       ios.ld_rambin(2)                                                          'Plexus starten
    else
...
pri testfenster|i,y,a
    ...
    a:=0
    window(1,4,2,1,27,38,string("Plexus-Flash-Updater"))
    rahmen (1,6,38,27)
    rahmen (1,1,38,3)
    rahmen (1,4,38,6)
    ...
    Print_win(string("Plexus-Updater for Hive-Computer 1.4"),2,2)
