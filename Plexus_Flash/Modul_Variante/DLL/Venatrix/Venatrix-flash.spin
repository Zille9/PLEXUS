...
PUB main
    ...
    testfenster
    util:=0
    testart:=0                                       'Testart=1-Wire
    repeat

      os_cmdinput                           'Hauptprogrammschleife

PRI os_cmdinput | x,y ,i,dk,a,b,c,port,key,padr,durchlauf,mpos
  durchlauf:=0
  repeat
'    durchlauf++
'    if durchlauf==121
...
'***************************** linke Maustaste **********************************************************************
    if ma==255
       ...
       ifnot infomarker
          if x>19 and x<36
                printfont(string("Value:    "),colors[winhcol],0,colors[act_color],20,25)
                printdec(busvalue[mpos],25,26,colors[winhcol],colors[act_color])
'****************************** Startmenue anzeigen ************************************************************
       if popupmarker==1
...
pri testfenster|i,y,a
    ...
    ios.displaytile(137,colors[winhcol],colors[winhcol],colors[act_color],6,17)
    ios.displaytile(136,colors[winhcol],colors[winhcol],colors[act_color],27,17)

    repeat i from 7 to 26
           ios.displaytile(2,colors[winhcol],colors[winhcol],colors[act_color],i,17)

    Print_win(string("Venatrix-Test for Hive-Computer V1.2"),2,2)
    Print_win(string("BUSGET:"),2,7)
    Print_win(string("PORTS :"),2,9)
    Print_win(string("DS1820:"),2,11)
    Print_win(string("SHT11 :"),2,13)
    Print_win(string("OTHER :"),2,15)
...
pri BUSGET|i,p
    ...
    repeat i from 0 to 31
          busvalue[i]:=ios.bus_getchar3

    repeat i from 0 to 15
           win_tile(6+((busvalue[i]&1)*123),8,20+i) '0
           win_tile(6+(((busvalue[i]>>1)&1)*123),10,20+i)'1
           win_tile(6+(((busvalue[i]>>2)&1)*123),12,20+i)'2
           win_tile(6+(((busvalue[i]>>3)&1)*123),14,20+i)'3
           win_tile(6+(((busvalue[i]>>4)&1)*123),16,20+i)'4
           win_tile(6+(((busvalue[i]>>5)&1)*123),18,20+i)'5
           win_tile(6+(((busvalue[i]>>6)&1)*123),20,20+i)'6
           win_tile(6+(((busvalue[i]>>7)&1)*123),22,20+i)'7
