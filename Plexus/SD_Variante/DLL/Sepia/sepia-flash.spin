...
16-03-2014       -DCF-Indikator in Titelleiste eingebaut
                 -Code optimiert
                 -6055 Longs frei
'############################################## HIVE-MAX-Version 1.3 ##############################################################################################################

03-05-2019       -Umfangreiche Änderungen in den plx-Routinen, da ein neuer IO-Chip verwendet wird (MCP23017)
                 -da es keine Analog-Ports mehr gibt, wurde die Funktionalität deaktiviert
                 -das Port schreiben wurde noch nicht getestet und muss eventuell angepasst werden
                 -5650 Longs frei
...
TXT_RAM        = $10A000'$20000          'Fehlertexte im Flash-Rom
'SETTING_RAM    = $7FF00 'Hier stehen die System-Settings
SETTING_ROM    = $100000 'Hier stehen die System-Settings
BRAM           = $40000
...
var
  ...
  byte PORT_IO[10]                  'In-Out-Richtungsmarker in=1 out=0
  byte PORT_BYTE[10]                 'BIT-Wert-Marker
  byte port_bit_old[10]             'temp-puffer
  byte bitmuster[10]                'bit-muster puffer
  byte bitfarbe[10]
  byte bit_muster4[10],bit_muster5[10],bit_muster6[10]   'Ausgabe-Bitmuster der Digital-Ports
       ...
       if infomarker==0
          ...
          if x=>23 and x=<30                                        'Portzustände
            if y==11 or y==13 or y==15
               ...
               ios.plxOut(PORT_ADR+port,!port_byte[port+4])
...
pri lese_register|i
{    Port_Byte[7]:=ios.get_Joyb
    Port_Byte[8]:=ios.get_Joya
    repeat i from 0 to 6
          PORT_Byte[i]:=!ios.getreg(i)
    zeige_register
    ios.serdec(port_byte[7])
    ios.sertx(32)
    ios.serdec(port_byte[8])
    ios.sertx(13)
}
          ...
          if port_io[i]==0          'Port auf Eingang?
             ...
             repeat b from 1 to 8
                  ...
                  if bitmuster[b]==0
                     ...
                     bitfarbe[b]:=red
             if port_byte[i]<>port_bit_old[i]                                       'nur geänderte werte anzeigen
                case i
                     0:balken(port_byte[0],21)
                     1:balken(port_byte[1],22)
                     2:balken(port_byte[2],23)
                     3:balken(port_byte[3],24)
'                     4:repeat a from 1 to 8
'                              Show_Joybit(bitfarbe[a],11,23+a-1)    'Port-Icon
'                     5:repeat a from 1 to 8
'                              Show_Joybit(bitfarbe[a],11,23+a-1)    'Port-Icon
...
pri port_setting|i,b
    ...
    repeat i from 0 to 2
           port_io[i]:=0
           ios.displaytile(138,colors[winhcol],colors[winhcol],colors[act_color],15+i-4+b,32)       'Digital-Ports IN anzeigen
           ios.displaytile(Cog_pic,colors[winhcol],colors[winhcol],colors[act_color],15+i-4+b,34)   'Radiobutton OUT
           b++
